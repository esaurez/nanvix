...
//==================================================================================================
// Thread Bootstrap Stub (moved to `sys-ffi` to keep it out of `libnvx_crt0.a`)
//==================================================================================================

// The `_do_start_thread` assembly entry point used to live here as a
// `global_asm!` block.  Because both `libnvx_crt0.a` and `libposix.a`
// link the `sys` crate, the `.global _do_start_thread` symbol ended up
// duplicated across both static archives, forcing every test ELF link
// line to use `-Wl,--allow-multiple-definition`.
//
// The asm now lives in the `sys-ffi` crate (`src/libs/sys-ffi/`),
// which is depended on only by `libposix`, so the symbol exists in a
// single archive.  `__kcall_create_thread` below still references
// `_do_start_thread` via `extern "C" fn`; the link resolves the symbol
// from `libposix.a` (sys-ffi) at link time.
