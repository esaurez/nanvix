...
//!
//! This crate is intentionally **stateless** — it has no `sysalloc`
//! dependency, no `extern crate alloc`, no heap usage, and no
//! `#[global_allocator]`.  All stateful runtime services (heap reservation,
//! `sysalloc` allocator setup, thread-data-area setup, argv / envp
//! parsing) live in `libposix`'s `start` module and are reached via the
//! `extern "C" fn __nanvix_libc_start_main` entry point.
//!
