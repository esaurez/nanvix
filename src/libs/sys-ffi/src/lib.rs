...
//! Nanvix `sys` C-ABI re-export shim.
//!
//! This crate exists solely to expose the kernel-call thunks defined in the
//! `sys::kcall` module hierarchy as unmangled (`#[no_mangle]`) globals so that
//! C consumers (libposix's C glue, cpython's `_nanvixmodule.c`, port-library
//! tests, etc.) can call them by their canonical names (`__kcall_*`,
//! `_do_exit_thread`, `__kcall_snapshot`).
//!
//! Splitting the FFI surface into its own crate avoids the duplicate-symbol
//! issue that arose when `libnvx_crt0.a` and `libposix.a` BOTH baked the
//! `sys` crate's `#[no_mangle]` exports into their static archives.  Now only
//! the consumers that explicitly depend on `sys-ffi` (notably `libposix`)
//! end up with the unmangled symbols in their archive; `libnvx_crt0` stays
//! FFI-clean.
//!
//! Each wrapper here is a thin trampoline that immediately forwards to the
//! Rust-mangled equivalent in `sys::kcall::*`.
...
//==================================================================================================
// Thread bootstrap stub (moved here from `sys::kcall::pm`)
//==================================================================================================
...
//
// The asm lives in `sys-ffi` (not `sys`) so the `.global _do_start_thread`
// symbol exists in a single static archive (`libposix.a`), avoiding the
// duplicate-strong-symbol clash that would otherwise occur when both
// `libnvx_crt0.a` and `libposix.a` link the `sys` crate.
//
