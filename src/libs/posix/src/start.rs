...
unsafe extern "C" {
    ...
    /// "KEY=VALUE" C strings.  Provided by `libposix` (this same crate)
    /// under the `syscall` feature; declared as an extern here so the
    /// reference does not require pulling the symbol's source module
    /// from the cargo dep graph.
...
///
#[unsafe(no_mangle)]
pub unsafe extern "C" fn __nanvix_libc_start_main(
    argp: *mut c_char,
    envp: *mut c_char,
) -> ! {
    ...
    // the heap is up.
    let argv: Vec<*const c_char> = unsafe { parse_argp(argp) };
    let argc: i32 = argv.len() as i32 - 1;
    let argv_ptr: *const *const u8 = argv.as_ptr() as *const *const u8;

    let mut env: Vec<*mut c_char> = unsafe { parse_envp(envp) };
    unsafe {
    ...
    // Keep the parsed argv / env vectors alive until after `main`
    // returns, so the application sees stable pointers throughout its
    // lifetime.  Explicit `drop` here documents the intent.
    drop(argv);
    drop(env);
    ...
    #[cfg(any(target_os = "none", target_os = "nanvix"))]
    {
        ...
        let heap_base: ::sys::mm::VirtualAddress = match ::sysalloc::vaddr::reserve(heap_capacity) {
            ...
            ::core::result::Result::Err(e) => {
                panic!(
                    "runtime_init(): failed to reserve virtual address space for heap: {:?}",
                    e
                )
            },
        };
...
unsafe fn build_string_table(string: *mut c_char) -> Vec<*mut c_char> {
    ...
    let mut result: Vec<*mut c_char> = Vec::with_capacity(count as usize);
    current = string;
    for _ in 0..count {
        ::syslog::trace!(
            "build_string_table(): entry[{}]: {:?}",
            result.len(),
            unsafe { ::core::ffi::CStr::from_ptr(current) }
        );
