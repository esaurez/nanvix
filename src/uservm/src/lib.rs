...
/// named pipe (`\\.\pipe\…` or `\\?\pipe\…`) the file is opened in write-only mode without
/// `create`/`truncate`, because Windows' `CreateFile` rejects those flags on a named pipe
/// with `ERROR_INVALID_PARAMETER` (os error 87). The shim that gave us this path is
/// responsible for creating the pipe server endpoint before we connect.
///
/// # Parameters
///
...
pub fn get_stderr_writer(vm_stderr: Option<String>) -> Result<Box<dyn Write + Send>> {
    ...
    let file_writer: Box<dyn Write + Send> = if let Some(vm_stderr) = vm_stderr {
        ...
        if is_windows_named_pipe(&vm_stderr) {
            let file = File::options().read(false).write(true).open(&vm_stderr)?;
...
///
/// Recognises both the canonical `\\.\pipe\` prefix and the rarer long-path
/// `\\?\pipe\` form. Matching is case-insensitive on the prefix only -- Windows
/// accepts pipe paths like `\\.\PIPE\foo`, `\\.\Pipe\foo`, etc. as equivalent
/// to the lower-case form -- but the pipe name itself is forwarded verbatim
/// to `CreateFile`.
///
/// # Out of scope
///
/// Remote UNC pipe paths (`\\<server>\pipe\<name>`) and forward-slash variants
/// (`//./pipe/<name>`) are intentionally not recognised: the in-tree callers
/// (the containerd shim and friends) always supply local-machine, backslash-
/// canonical paths. If a future caller needs remote pipes, the helper should
/// be extended together with the test suite below.
///
/// # Parameters
///
...
///
#[cfg(windows)]
pub(crate) fn is_windows_named_pipe(path: &str) -> bool {
    // Only inspect the first 9 ASCII bytes (length of `\\.\pipe\` / `\\?\pipe\`).
    // We deliberately avoid lower-casing the entire path: pipe names themselves
    // should be passed verbatim to `CreateFile`, and large paths should not
    // allocate just to be classified.
    const PREFIX_LEN: usize = r"\\.\pipe\".len();
    let bytes: &[u8] = path.as_bytes();
    if bytes.len() < PREFIX_LEN {
        return false;
    }
    let head: &[u8] = &bytes[..PREFIX_LEN];
    head.eq_ignore_ascii_case(br"\\.\pipe\") || head.eq_ignore_ascii_case(br"\\?\pipe\")
}
