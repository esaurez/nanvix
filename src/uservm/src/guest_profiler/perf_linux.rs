...
impl PerfSession {
    ...
    ///
    /// Uses system-wide sampling (`-a`) to capture kernel stacks (kvm, ioctl,
    /// etc.) that are invisible with `-p PID` (which implicitly sets
    /// `exclude_kernel=1`). Post-processing filters stacks to the nanvixd
    /// process. Requires root or CAP_PERFMON; falls back to `-p` if `-a` fails.
