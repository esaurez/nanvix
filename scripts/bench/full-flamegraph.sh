...
# sends SIGUSR2 to the vCPU thread and walks the frame-pointer
...
export NANVIX_GUEST_PROFILE_PATH="$GUEST_FOLDED"
...
# Host stacks from perf. When perf runs system-wide (-a), we filter to
# the nanvixd process by grepping collapsed stacks for the binary or
# crate name (nanvixd or uservm -- Rust symbols use the crate name).
# This is more robust than --comms (Linux truncates comm to 15 chars).
#
# We use `perf script -F` to omit the period field -- otherwise
# inferno-collapse-perf accumulates raw event periods (~1M per cpu-clock
# sample) which would dwarf guest sample counts (1 per sample).
# Without the period, each perf event collapses to count=1, matching
# the guest profiler's 1-per-sample weighting.
if [ -f "$PERF_DATA" ] && command -v perf &>/dev/null; then
    ...
    perf script -i "$PERF_DATA" -F comm,pid,tid,time,event,ip,sym,dso 2>/dev/null | \
        inferno-collapse-perf 2>/dev/null | \
        grep -iE "nanvixd|uservm" | \
